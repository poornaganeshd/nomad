...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], onLearnRule = () => {}, wallets: aw = WALLETS, cloudinaryEnabled = false, onLocalReceipt }) {
  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type !== "recurring" && (() => { const handleVoice = t => { const r = parseVoiceTx(t, { wallets: aw, categories: type === "expense" ? cats : isrc }); if (r.amount) sAmt(String(r.amount)); if (r.note) sNote(r.note); if (r.walletId) { if (type === "expense") sW(r.walletId); else if (!isUpiLite(aw.find(w => w.id === r.walletId) || {})) sIW(r.walletId); } if (r.categoryId) { if (type === "expense") sCat(r.categoryId); else sSrc(r.categoryId); } showT(r.amount ? `Heard: ₹${r.amount} ${r.note || ""}` : "Couldn't parse — try \"300 coffee bank\"", r.amount ? "info" : "error"); }; return <>
      {type === "expense" && patterns.length > 0 && <div style={{ marginBottom: 14 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 10, color: "var(--muted)", letterSpacing: "1.5px", fontWeight: 700, marginBottom: 8 }}>QUICK ADD</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: 6 }}>{patterns.slice(0, 6).map((p, i) => { const cat = cats.find(c => c.id === p.categoryId); const accent = cat?.color || "#E07A5F"; return <button key={i} onClick={() => { sAmt(String(p.amount)); sCat(p.categoryId); sW(p.walletId); if (p.note) sNote(p.note); }} style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 10px", borderRadius: 12, border: `1.5px solid ${accent}`, background: accent + "12", cursor: "pointer", fontFamily: "var(--font-h)", minWidth: 0 }}><DI2 id={p.categoryId} accent={cat?.neon || accent} size={13} /><span style={{ fontSize: 12, fontWeight: 700, color: accent, flexShrink: 0 }}>₹{p.amount}</span>{p.note && <span style={{ fontSize: 10, color: "var(--ts)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, minWidth: 0 }}>{p.note}</span>}</button>; })}</div></div>}
      <div style={{ background: "var(--card)", border: `1.5px solid ${tc}`, borderRadius: 18, padding: "10px 12px", marginBottom: 14, boxShadow: `0 2px 10px ${tc}20` }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ flex: 1, display: "flex", alignItems: "center", minWidth: 0 }}>{type !== "transfer" && <span style={{ fontSize: 26, lineHeight: 1, flexShrink: 0, color: tc, fontFamily: "var(--font-h)", fontWeight: 700, marginRight: 4 }}>₹</span>}<input type="number" value={amt === "0" ? "" : amt} onChange={e => sAmt(e.target.value || "0")} placeholder="0" autoFocus style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 30, fontWeight: 700, fontFamily: "var(--font-h)", padding: "8px 4px", color: tc, minWidth: 0, width: "100%" }} /></div><div style={{ display: "flex", gap: 6, flexShrink: 0, alignItems: "center" }}>{(type === "expense" || type === "income") && <VoiceAdd compact accent={tc} onParsed={handleVoice} />}{type !== "transfer" && !fxExpanded && <button onClick={() => setFxExpanded(true)} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "0 10px", height: 40, borderRadius: 12, border: `1.5px solid ${fxCur !== "INR" ? tc : "var(--border)"}`, background: fxCur !== "INR" ? tc + "14" : "var(--bg)", cursor: "pointer", fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 12, color: fxCur !== "INR" ? tc : "var(--muted)", letterSpacing: 0.5, flexShrink: 0 }}>{fxCur}<svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" style={{ opacity: 0.6 }}><polyline points="9 18 15 12 9 6" /></svg></button>}{type !== "transfer" && fxExpanded && <button onClick={() => { setFxExpanded(false); setFxSearch(""); }} style={{ width: 40, height: 40, padding: 0, borderRadius: 12, border: "1.5px solid var(--border)", background: "var(--bg)", cursor: "pointer", fontFamily: "var(--font-h)", fontSize: 14, color: "var(--muted)", flexShrink: 0 }}>✕</button>}</div></div></div>
      {type !== "transfer" && fxExpanded && <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 12, marginBottom: 14 }}><input value={fxSearch} onChange={e => setFxSearch(e.target.value)} placeholder="Search currency or country…" autoFocus style={{ ...is, marginBottom: 10, padding: "8px 12px", fontSize: 13 }} /><div style={{ display: "flex", flexWrap: "wrap", gap: 6, maxHeight: 200, overflowY: "auto" }}>{(fxSearch.trim() ? CURRENCIES.filter(c => { const q = fxSearch.trim().toLowerCase(); return c.toLowerCase().includes(q) || (CURRENCY_COUNTRIES[c] || "").toLowerCase().includes(q); }) : CURRENCIES).map(c => <button key={c} onClick={() => { setFxCur(c); setFxExpanded(false); setFxSearch(""); }} style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "7px 10px", borderRadius: 10, border: `1.5px solid ${fxCur === c ? tc : "var(--border)"}`, background: fxCur === c ? tc + "18" : "var(--bg)", color: fxCur === c ? tc : "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: fxCur === c ? 700 : 500, whiteSpace: "nowrap", cursor: "pointer", flexShrink: 0 }}>{c}{CURRENCY_COUNTRIES[c] && <span style={{ fontSize: 9, opacity: 0.65, fontWeight: 400, maxWidth: 60, overflow: "hidden", textOverflow: "ellipsis" }}>{CURRENCY_COUNTRIES[c]}</span>}</button>)}</div>{fxFetching && <div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-h)", marginTop: 8 }}>Fetching rate…</div>}{!fxFetching && fxRate && fxCur !== "INR" && <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, paddingTop: 8, borderTop: "1px solid var(--border)" }}>{parseFloat(amt) > 0 && <span style={{ fontSize: 14, color: tc, fontFamily: "var(--font-h)", fontWeight: 700 }}>≈ {fmt(roundMoney(parseFloat(amt) * fxRate))}</span>}<span style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 500 }}>1 {fxCur} = ₹{fxRate.toFixed(2)}</span></div>}</div>}
      {type !== "transfer" && !fxExpanded && fxCur !== "INR" && <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: -8, marginBottom: 14, padding: "0 4px" }}>{fxFetching && <span style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-h)" }}>Fetching rate…</span>}{!fxFetching && fxRate && <><span style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 500 }}>1 {fxCur} = ₹{fxRate.toFixed(2)}</span>{parseFloat(amt) > 0 && <span style={{ fontSize: 13, fontFamily: "var(--font-h)", fontWeight: 700, color: tc }}>≈ {fmt(roundMoney(parseFloat(amt) * fxRate))}</span>}</>}</div>}
      <div style={{ position: "relative", background: "var(--card)", borderRadius: 18, padding: "16px 16px 8px 22px", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.04)", border: "1px solid var(--border)" }}><div style={{ position: "absolute", left: 0, top: 14, bottom: 14, width: 4, background: tc, borderRadius: "0 3px 3px 0" }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 10, color: tc, letterSpacing: "1.8px", fontWeight: 800, marginBottom: 14 }}>{type === "expense" ? "PAYMENT" : type === "income" ? "SOURCE" : "TRANSFER"}</div>
        {type === "expense" && <><label style={ls}>Pay From</label><WB wallets={aw} sel={wid} onSel={sW} /><label style={ls}>Category</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>{[...cats, ...(cats.find(c => c.id === "other") ? [] : [DC.find(c => c.id === "other")])].map(c => <button key={c.id} onClick={() => sCat(c.id)} style={{ padding: "8px 12px", borderRadius: 10, fontSize: 13, fontFamily: "var(--font-b)", border: `1.5px solid ${catId === c.id ? c.color : "var(--border)"}`, background: catId === c.id ? c.color + "18" : "var(--bg)", color: catId === c.id ? c.color : "var(--ts)", cursor: "pointer", fontWeight: catId === c.id ? 600 : 400, display: "flex", alignItems: "center", gap: 5 }}><DI2 id={c.id} accent={c.neon || c.color} size={14} />{c.name}</button>)}</div></>}
        {type === "income" && <><label style={ls}>Receive Into</label><WB wallets={aw.filter(w => !isUpiLite(w))} sel={iwid} onSel={sIW} /><label style={ls}>Source</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>{isrc.map(c => <button key={c.id} onClick={() => sSrc(c.id)} style={{ padding: "8px 12px", borderRadius: 10, fontSize: 13, fontFamily: "var(--font-b)", border: `1.5px solid ${srcId === c.id ? c.color : "var(--border)"}`, background: srcId === c.id ? c.color + "18" : "var(--bg)", color: srcId === c.id ? c.color : "var(--ts)", cursor: "pointer", fontWeight: srcId === c.id ? 600 : 400, display: "flex", alignItems: "center", gap: 5 }}><DI2 id={c.id} accent={c.neon || c.color} size={14} />{c.name}</button>)}</div></>}
        {type === "transfer" && <><label style={ls}>From</label><WB wallets={aw} sel={tFrom} onSel={sTF} /><div style={{ textAlign: "center", fontSize: 18, color: tc, marginBottom: 10, fontWeight: 700 }}>↓</div><label style={ls}>To</label><WB wallets={aw} sel={tTo} onSel={sTT} />{tFrom === tTo && <p style={{ fontSize: 12, color: "#D4726A", textAlign: "center", marginBottom: 8 }}>Must be different.</p>}</>}
      </div>
      <div style={{ position: "relative", background: "var(--card)", borderRadius: 18, padding: "16px 16px 8px 22px", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.04)", border: "1px solid var(--border)" }}><div style={{ position: "absolute", left: 0, top: 14, bottom: 14, width: 4, background: "var(--muted)", borderRadius: "0 3px 3px 0", opacity: 0.4 }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 10, color: "var(--muted)", letterSpacing: "1.8px", fontWeight: 800, marginBottom: 14 }}>DETAILS</div>
        <div style={{ display: "flex", gap: 10, marginBottom: aiCatSug || aiCatLoading ? 6 : 14 }}><div style={{ flex: 1 }}><label style={ls}>Date</label><input type="date" value={date} onChange={e => sDate(e.target.value)} style={is} /></div><div style={{ flex: 1 }}><label style={ls}>Note</label><input value={note} onChange={e => { const v = e.target.value; sNote(v); sAiCatSug(null); if (aiDebounceRef.current) clearTimeout(aiDebounceRef.current); if (type === "expense") { const kw = v.toLowerCase().trim(); const m = autoRules.find(r => kw.includes(r.keyword.toLowerCase())); if (m) { sCat(m.categoryId); } else if (v.trim().length >= 3) { aiDebounceRef.current = setTimeout(async () => { sAiCatLoading(true); try { const r = await fetch("/api/ai-categorize", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ note: v.trim(), categories: cats.map(c => ({ id: c.id, name: c.name })) }) }); const d = await r.json(); if (r.ok && d.categoryId) sAiCatSug({ ...d, keyword: extractKeyword(v) }); } catch { /* silent */ } finally { sAiCatLoading(false); } }, 800); } } }} placeholder="Optional…" style={is} /></div></div>
        {aiCatLoading && <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 10, display: "flex", alignItems: "center", gap: 5 }}>AI suggesting category…</div>}
        {aiCatSug && (() => { const c = cats.find(x => x.id === aiCatSug.categoryId); if (!c) return null; return <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, padding: "7px 10px", borderRadius: 10, background: (c.color || "#6BAA75") + "12", border: `1px solid ${c.color || "#6BAA75"}` }}><DI2 id={c.id} accent={c.neon || c.color} size={14} /><span style={{ flex: 1, fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)", display: "flex", alignItems: "center", gap: 4 }}><Robot size={12} />{c.name}?</span><span style={{ fontSize: 9, color: c.color, background: (c.color || "#6BAA75") + "20", padding: "1px 5px", borderRadius: 4, fontWeight: 700 }}>{aiCatSug.confidence}</span><button onClick={() => { sCat(aiCatSug.categoryId); onLearnRule({ keyword: aiCatSug.keyword, categoryId: aiCatSug.categoryId, source: "ai", confidence: 0.9, hitCount: 0, createdAt: localDateKey() }); sAiCatSug(null); }} style={{ padding: "4px 10px", borderRadius: 6, border: "none", background: c.color || "#6BAA75", color: "#fff", fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>✓</button><button onClick={() => sAiCatSug(null)} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 11, cursor: "pointer" }}>✗</button></div>; })()}
        {type !== "transfer" && <div style={{ marginBottom: 10 }}><ReceiptPicker ref={receiptPickerRef} cloudinaryEnabled={cloudinaryEnabled} />{type === "expense" && <button onClick={scanReceipt} disabled={ocrLoading} style={{ marginTop: 8, width: "100%", padding: "9px 12px", border: `1.5px dashed ${tc}`, borderRadius: 10, background: ocrLoading ? "var(--border)" : "var(--bg)", color: tc, fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: ocrLoading ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><Receipt size={14} weight="fill" />{ocrLoading ? "Scanning…" : "Scan receipt — auto-fill"}</button>}</div>}
      </div>
      <button onClick={submit} disabled={submitting} style={{ width: "100%", padding: "16px", border: "none", borderRadius: 14, background: submitting ? tc + "99" : tc, color: "#fff", fontSize: 15, fontFamily: "var(--font-h)", fontWeight: 700, letterSpacing: 0.5, cursor: submitting ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: `0 6px 16px ${tc}40`, marginTop: 4 }}>
