...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onSettleEventNet: oSEN, onDeleteSplit: oDS, onDeleteExp: oDE, onMarkDone: oMD, onReopen: oRO, onDelete: oD, onUpdate: oU, onToast: showT = () => {}, dm = false }) {
  ...
  const goDetail = (id) => { sSel(id); sEPaidBy("me"); sEA(""); sEN(""); sEC(cats[0]?.id || ""); sEW(wl[0]?.id || "bank"); sSN(""); sSA(""); sSPNote(""); sSD("owed"); sSTgt(null); sBsO(false); sBsS(1); sBsM("equal"); sBsT(""); sBsP([{ name: "", amount: "" }]); sBsN(""); sBsC(cats[0]?.id || ""); sBsW(wl[0]?.id || "bank"); sEvDelConfirm(null); sDoneConfirm(false); sSuTgt(null); sEditOpen(false); sShowSettled(false); sV("detail"); };
  ...
  if (view === "create") return <div style={{ position: "relative", background: pC, height: "calc(100vh - 90px)", display: "flex", flexDirection: "column", overflow: "hidden" }}><TopoBg />
  ...
  if (view === "detail" && sel) {
    ...
    return <div style={{ position: "relative", background: pC, height: "calc(100vh - 90px)", display: "flex", flexDirection: "column", overflow: "hidden" }}><TopoBg />
      ...
      {eExps.length > 0 && (() => { const m = {}; eExps.forEach(e => { m[e.categoryId] = (m[e.categoryId] || 0) + e.amount; }); const rows = Object.entries(m).map(([cid, amt]) => ({ cat: cats.find(c => c.id === cid) || { id: cid, name: "Other", color: "#8A8A9A" }, amt })).sort((a, b) => b.amt - a.amt); const top = rows.slice(0, 4), rest = rows.length - top.length; return <div style={{ padding: "0 2px", marginBottom: 26, animation: "evSlideUp 0.4s 0.08s ease both" }}><div style={{ display: "flex", height: 6, borderRadius: 4, overflow: "hidden", gap: 2, marginBottom: 8 }}>{rows.map(r => <div key={r.cat.id} style={{ width: `${Math.max(2, r.amt / tp * 100)}%`, background: r.cat.color, borderRadius: 4, opacity: 0.85 }} />)}</div><div style={{ display: "flex", flexWrap: "wrap", gap: "4px 14px" }}>{top.map(r => <span key={r.cat.id} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 10.5, color: mutC }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: r.cat.color, flexShrink: 0 }} />{r.cat.name}<span style={{ color: inkC, fontWeight: 600 }}>{fmt(r.amt)}</span></span>)}{rest > 0 && <span style={{ fontSize: 10.5, color: mutC }}>+{rest} more</span>}</div></div>; })()}
      {eExps.length === 0 && eSp.length === 0 && <div style={{ textAlign: "center", padding: "24px 20px 28px", animation: "evSlideUp 0.4s 0.1s ease both" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: 10, color: mutC, opacity: 0.55 }}><Compass size={32} /></div><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 16, color: inkC, opacity: 0.7 }}>Nothing logged yet</div><div style={{ fontSize: 11.5, color: mutC, marginTop: 4, lineHeight: 1.5 }}>{sel.status === "active" ? "Add the first expense below to start the trail." : "No spending was recorded for this event."}</div></div>}
