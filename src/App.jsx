...
function Splits({ splits: sp, settlements: stl, onAdd, onSettle: os, onDelete: od, expanded: exp, onToggle: ot, wallets: wl = WALLETS }) {
  ...
  if(exp&&drill){
    ...
    {events.map(ev=>ev._k==="split"?<div key={ev.id} style={{borderRadius:12,marginBottom:8,border:`1px solid ${ev.settled?"var(--border)":ev.direction==="owe"?"#E07A5F30":"#6BAA7530"}`,overflow:"hidden",opacity:ev.settled?0.6:1}}><div style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",background:"var(--bg)"}}>{ev.settled?<IconCheck size={16} color="#6BAA75"/>:ev.direction==="owe"?<IconArrowDown size={16} color="#E07A5F"/>:<IconArrowUp size={16} color="#6BAA75"/>}<div style={{flex:1}}><div style={{fontSize:13,fontFamily:"var(--font-h)",fontWeight:600,color:"var(--text)"}}>{ev.direction==="owe"?"You owe":"Owes you"}{ev.note&&<span style={{opacity:0.6,fontWeight:400,fontSize:12}}> · {ev.note}</span>}</div><div style={{fontSize:10,color:"var(--muted)",fontFamily:"var(--font-b)",marginTop:1,display:"flex",alignItems:"center",gap:4}}>{ev.settled?<><IconCheck size={9} color="#6BAA75"/>Settled</>:<><IconClock size={9} color="var(--muted)"/>Pending{ev.createdAt&&` · ${fmtDate(ev.createdAt)}`}</>}</div></div><div style={{textAlign:"right"}}><div style={{fontFamily:"var(--font-h)",fontWeight:700,fontSize:14,color:ev.direction==="owe"?"#E07A5F":"#6BAA75"}}>{fmt(ev.amount)}</div>{!ev.settled&&<div style={{fontSize:9,color:"var(--muted)",fontFamily:"var(--font-h)"}}>{fmt(remainForSplit(ev))} left</div>}</div></div>{!ev.settled&&delConfirm===ev.id&&<div style={{background:"#E07A5F12",padding:"10px 14px",display:"flex",alignItems:"center",gap:10,borderTop:"1px solid #E07A5F30"}}><IconAlertTriangle size={14} color="#E07A5F"/><span style={{flex:1,fontSize:11,fontFamily:"var(--font-h)",color:"#E07A5F"}}>Delete this IOU permanently?</span><button onClick={()=>sDelConfirm(null)} style={{padding:"4px 10px",border:"1px solid var(--border)",borderRadius:6,background:"transparent",color:"var(--muted)",fontFamily:"var(--font-h)",fontSize:10,cursor:"pointer"}}>Cancel</button><button onClick={()=>{od(ev.id);sDelConfirm(null);}} style={{padding:"4px 10px",border:"none",borderRadius:6,background:"#E07A5F",color:"#fff",fontFamily:"var(--font-h)",fontSize:10,fontWeight:700,cursor:"pointer"}}>Delete</button></div>}{!ev.settled&&<div style={{display:"flex",borderTop:"1px solid var(--border)"}}><button onClick={()=>sT(ev)} style={{flex:1,padding:"8px",border:"none",background:"transparent",color:"#6BAA75",fontFamily:"var(--font-h)",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><IconSend size={12}/>Record payment</button><div style={{width:1,background:"var(--border)"}}/><button onClick={()=>sDelConfirm(delConfirm===ev.id?null:ev.id)} style={{padding:"8px 16px",border:"none",background:"transparent",color:"var(--muted)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><IconTrash size={13}/></button></div>}</div>:<div key={ev.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--bg)",borderRadius:12,marginBottom:8,border:"1px solid var(--border)",opacity:0.65}}><div style={{width:28,height:28,borderRadius:8,background:ev.direction==="owed"?"#6BAA7518":"#E07A5F18",display:"flex",alignItems:"center",justifyContent:"center"}}><IconSend size={13} color={ev.direction==="owed"?"#6BAA75":"#E07A5F"}/></div><div style={{flex:1}}><div style={{fontSize:12,fontFamily:"var(--font-h)",fontWeight:600,color:"var(--text)"}}>{ev.direction==="owed"?"Payment received":"Payment made"}</div><div style={{fontSize:10,color:"var(--muted)",fontFamily:"var(--font-b)"}}>{ev.createdAt?fmtDate(ev.createdAt):ev.date||""}</div></div><span style={{fontFamily:"var(--font-h)",fontWeight:700,fontSize:14,color:ev.direction==="owed"?"#6BAA75":"#E07A5F"}}>{ev.direction==="owed"?"+":"-"}{fmt(ev.amount)}</span></div>)}
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <Splits splits={sp} settlements={stl} expanded={spX} onToggle={() => sSpX(!spX)} onAdd={s => { const sr = { ...s, createdAt: new Date().toISOString() }; sSp(p => [...p, sr]); sbUpsert("splits", [toSB(sr, ["id", "name", "amount", "direction", "settled", "eventId", "groupId", "note"])]) }} onSettle={settle} onDelete={id => { sSp(p => p.filter(s => s.id !== id)); sbDelete("splits", id); }} wallets={wallets} />
