...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onDeleteSplit: oDS, onMarkDone: oMD, onDelete: oD, dm = false }) {
  ...
  if (view === "detail" && sel) {
    ...
    return <div style={{ position: "relative", background: pC, height: "calc(100vh - 90px)", display: "flex", flexDirection: "column", overflow: "hidden" }}><TopoBg />
      ...
      <div style={{ background: stripC, borderRadius: 14, padding: "14px 6px", display: "flex", alignItems: "center", marginBottom: 20, animation: "evSlideUp 0.4s 0.05s ease both" }}>{(isGroup ? [[fmt(grpTotal), "Group Total"], [fmt(grpShare), "Your Share"], [String(allParts.length), "People"]] : [[fmt(ns), "Net Spent"], [fmt(tp), "Total Paid"], [String(eExps.length), "Entries"]]).map(([v, l], i) => <div key={l} style={{ flex: 1, textAlign: "center", borderLeft: i > 0 ? "1px solid rgba(255,255,255,0.1)" : "none" }}><div style={{ fontSize: 15, fontWeight: 500, color: "#C9A84C", fontFamily: "'Playfair Display', Georgia, serif" }}>{v}</div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.4)", letterSpacing: "0.8px", textTransform: "uppercase", marginTop: 2 }}>{l}</div></div>)}</div>
      ...
      {isGroup && allParts.length > 1 && <div style={{ ...wcc, padding: "2px 16px", marginBottom: 20, animation: "evSlideUp 0.4s 0.1s ease both" }}><div style={{ fontSize: 10.5, color: mutC, padding: "11px 0", borderBottom: `1px solid ${brdC}`, letterSpacing: "0.3px" }}>Equal share {fmt(grpShare)} per person · {allParts.length} people</div>{allParts.map(p => { const paid = grpPaid[p] || 0, bal = roundMoney(paid - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)); const settledP = Math.abs(bal) < 0.01; const av = avColor(p); return <div key={p} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: `1px solid ${brdC}` }}><div style={{ width: 32, height: 32, borderRadius: "50%", background: av + "1F", border: `1px solid ${av}40`, color: av, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10.5, fontWeight: 700, flexShrink: 0, letterSpacing: "0.5px" }}>{initialsOf(p)}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{p}</div><div style={{ fontSize: 10.5, color: mutC, marginTop: 1 }}>paid {fmt(paid)} · share {fmt(grpShareMap[p] ?? grpShare)}</div></div><div style={{ textAlign: "right", flexShrink: 0 }}>{settledP ? <span style={{ fontSize: 11, color: mutC, display: "inline-flex", alignItems: "center", gap: 4 }}><IconCheck size={11} />settled</span> : <><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: bal > 0 ? greenC : terraC }}>{fmt(Math.abs(bal))}</div><div style={{ fontSize: 9, letterSpacing: "0.8px", textTransform: "uppercase", color: bal > 0 ? greenC : terraC, marginTop: 1 }}>{bal > 0 ? "gets back" : "owes"}</div></>}</div></div>; })}{(() => { const bals = allParts.map(p => ({ name: p, bal: roundMoney((grpPaid[p] || 0) - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)) })); const cr = bals.filter(b => b.bal > 0.01).map(b => ({ ...b })).sort((a, b) => b.bal - a.bal); const db = bals.filter(b => b.bal < -0.01).map(b => ({ ...b })).sort((a, b) => a.bal - b.bal); const settlements = []; let ci = 0, di = 0; while (ci < cr.length && di < db.length) { const amt = roundMoney(Math.min(cr[ci].bal, -db[di].bal)); settlements.push({ from: db[di].name, to: cr[ci].name, amt }); cr[ci].bal = roundMoney(cr[ci].bal - amt); db[di].bal = roundMoney(db[di].bal + amt); if (cr[ci].bal < 0.01) ci++; if (db[di].bal > -0.01) di++; } return settlements.length > 0 && <div style={{ padding: "10px 0 12px" }}><div style={{ fontSize: 9, letterSpacing: "1.5px", textTransform: "uppercase", color: goldC, fontWeight: 600, marginBottom: 8 }}>Settle Up</div>{settlements.map((s, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, marginBottom: i < settlements.length - 1 ? 7 : 0 }}><span style={{ fontWeight: 600, color: s.from === "You" ? terraC : inkC }}>{s.from}</span><ArrowRight size={12} color={mutC} /><span style={{ fontWeight: 600, color: s.to === "You" ? greenC : inkC }}>{s.to}</span><span style={{ marginLeft: "auto", fontFamily: "'Playfair Display', Georgia, serif", fontSize: 14, color: inkC }}>{fmt(s.amt)}</span></div>)}</div>; })()}</div>}
      ...
      {eSp.length > 0 && <div style={{ ...wcc, padding: "2px 16px", marginBottom: 20, animation: "evSlideUp 0.4s 0.18s ease both" }}>{eSp.map((s, si) => <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: si < eSp.length - 1 ? `1px solid ${brdC}` : "none", opacity: s.settled ? 0.45 : 1 }}><div style={{ width: 34, height: 34, borderRadius: "50%", background: s.settled || s.direction !== "owe" ? "rgba(90,122,90,0.1)" : "rgba(192,82,74,0.1)", border: `1px solid ${s.settled || s.direction !== "owe" ? "rgba(90,122,90,0.25)" : "rgba(192,82,74,0.25)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: s.settled || s.direction !== "owe" ? greenC : redC }}>{s.settled ? <CheckCircle size={16} weight="fill" /> : s.direction === "owe" ? <ArrowUpRight size={15} /> : <ArrowDownLeft size={15} />}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{s.name}</div><div style={{ fontSize: 10.5, color: mutC, marginTop: 1 }}>{s.settled ? "Settled" : s.direction === "owe" ? "You owe" : "Owes you"}</div>{s.note && <div style={{ fontSize: 10, color: mutC, marginTop: 1, fontStyle: "italic", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.note}</div>}</div><span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: s.direction === "owe" ? redC : greenC, flexShrink: 0 }}>{fmt(s.amount)}</span>{!s.settled && <button onClick={() => sSTgt(s)} style={{ border: "1px solid rgba(184,150,62,0.4)", background: "rgba(184,150,62,0.08)", borderRadius: 20, padding: "4px 11px", fontSize: 10, fontWeight: 600, letterSpacing: "0.5px", color: goldC, cursor: "pointer", flexShrink: 0 }}>Settle</button>}<button onClick={() => oDS(s.id)} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 0, display: "flex" }}><IconX size={14} /></button></div>)}</div>}
      ...
      {sel.status === "active" && !isGroup && (() => {
        ...
        if (!bsOpen) return <button onClick={() => sBsO(true)} style={{ width: "100%", padding: 14, border: "1.5px dashed rgba(184,150,62,0.55)", borderRadius: 14, background: "rgba(184,150,62,0.06)", color: goldC, fontSize: 13, fontWeight: 600, letterSpacing: "0.5px", cursor: "pointer", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}><Receipt size={15} />Bill Splitter</button>;
        ...
        return <div style={{ ...wcc, padding: 16, marginBottom: 20 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><div style={{ ...secLbl, padding: 0 }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Receipt size={11} />Bill Splitter</span></div><button onClick={bsReset} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", display: "flex", padding: 0 }}><IconX size={14} /></button></div>
          ...
          <button onClick={() => { if (canSub) sBsS(2) }} disabled={!canSub} style={{ width: "100%", padding: 13, border: "none", borderRadius: 10, background: canSub ? greenC : stoneC, color: canSub ? "#fff" : mutC, fontSize: 13, fontWeight: 700, cursor: canSub ? "pointer" : "default", opacity: canSub ? 1 : 0.7, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, boxShadow: canSub ? "0 2px 8px rgba(90,122,90,0.3)" : "none" }}>Review Split<ArrowRight size={13} /></button></div>
      })()}
      ...
      {sel.status === "active" && <div style={{ ...wcc, padding: 16, marginBottom: 20 }}><div style={{ display: "flex", gap: 6, marginBottom: 12 }}>{["owed", "owe"].map(d => <button key={d} onClick={() => sSD(d)} style={{ flex: 1, padding: "9px", borderRadius: 10, fontSize: 12, border: `1px solid ${sd === d ? (d === "owe" ? redC : greenC) : brdC}`, background: sd === d ? (d === "owe" ? "rgba(192,82,74,0.08)" : "rgba(90,122,90,0.08)") : "transparent", color: sd === d ? (d === "owe" ? redC : greenC) : mutC, cursor: "pointer", fontWeight: sd === d ? 600 : 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>{d === "owe" ? <ArrowUpRight size={13} /> : <ArrowDownLeft size={13} />}{d === "owe" ? "I owe them" : "They owe me"}</button>)}</div><div style={{ display: "flex", gap: 8 }}><input value={sn} onChange={e => sSN(e.target.value)} placeholder="Friend name" style={{ ...wis, flex: 1, width: "auto" }} /><input type="number" value={sa} onChange={e => sSA(e.target.value)} placeholder="₹" style={{ ...wis, width: 74 }} /><input value={spNote} onChange={e => sSPNote(e.target.value)} placeholder="Note" style={{ ...wis, flex: 1, width: "auto" }} /><button onClick={addSplit} style={{ padding: "10px 15px", border: "none", borderRadius: 10, background: greenC, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", boxShadow: "0 2px 8px rgba(90,122,90,0.3)" }}><IconPlus size={16} /></button></div></div>}
      </div>
