...
function Splits({ splits: sp, settlements: stl, categories: cats = [], onAdd, onSettle: os, onSettleNet: osn = () => {}, onDelete: od, onSkip: ok, onWriteOff: owf = () => {}, expanded: exp, onToggle: ot, wallets: wl = WALLETS, walletBalances: wBalances = {}, onError = () => {} }) {
  ...
  const [sdate, sSdate] = useState(localDateKey());
  ...
  const add = () => { if((!nm.trim()&&!drill)||!am) return; const a=parseAmount(am); if(!Number.isFinite(a)||a<=0) return; onAdd({id:uid(),name:drill||nm.trim(),amount:a,direction:dir,settled:false,note:snote.trim()||undefined,categoryId:effScat||undefined,date:sdate||localDateKey()}); sN(""); sA(""); sSnote(""); sSdate(localDateKey()); };
  ...
  const fmtDate = ts => { try{ const d = (typeof ts==="string"&&/^\d{4}-\d{2}-\d{2}$/.test(ts)) ? new Date(ts+"T12:00:00") : new Date(ts); return d.toLocaleDateString("en-IN",{month:"short",day:"numeric"});}catch{return "";} };
  // IOU date key: the IOU's own `date` (backdated), else fall back to creation
  // stamp → so a split logged for last week sorts into its real chronological
  // slot, not where the row was created. Both splits and settlements carry `date`.
  const iouKey = it => it.date || (it.createdAt?localDateKey(new Date(it.createdAt)):(it.created_at?localDateKey(new Date(it.created_at)):""));
  const iouSort = (a,b) => { const k=iouKey(b).localeCompare(iouKey(a)); return k!==0?k:itemTimestamp(b)-itemTimestamp(a); };
  ...
  if(exp&&drill){
    ...
    const events=[...pm.splits.map(s=>({...s,_k:"split"})),...pStls.map(s=>({...s,_k:"stl"}))].sort(iouSort);
