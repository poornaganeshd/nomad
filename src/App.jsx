...
export default function Nomad() {
  ...
  const weeklyDigest = useMemo(() => {
    const today = localDateKey(); const d7 = new Date(today); d7.setDate(d7.getDate() - 6); const from = d7.toISOString().slice(0, 10);
    const wEx = ex.filter(e => e.date >= from && e.date <= today);
    const wInc = inc.filter(i => i.date >= from && i.date <= today);
    const spent = roundMoney(wEx.reduce((s, e) => s + e.amount, 0));
    const earned = roundMoney(wInc.reduce((s, i) => s + i.amount, 0));
    const topCat = Object.entries(wEx.reduce((m, e) => { m[e.categoryId] = (m[e.categoryId] || 0) + e.amount; return m; }, {})).sort((a, b) => b[1] - a[1])[0];
    return { spent, earned, days: wEx.length + wInc.length > 0 ? 7 : 0, topCat };
  }, [ex, inc]);
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        {weeklyDigest.days > 0 && <div style={{ ...cc, padding: "12px 16px", marginBottom: 12, borderLeft: "3px solid #6BAA75" }}><div style={{ fontFamily: "var(--font-h)", fontSize: 11, color: "#6BAA75", fontWeight: 700, letterSpacing: "0.5px", marginBottom: 8 }}>THIS WEEK (7 DAYS)</div><div style={{ display: "flex", gap: 16 }}><div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-h)" }}>Spent</div><div style={{ fontSize: 16, fontWeight: 700, fontFamily: "var(--font-h)", color: "#E07A5F" }}>{fmt(weeklyDigest.spent)}</div></div><div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-h)" }}>Earned</div><div style={{ fontSize: 16, fontWeight: 700, fontFamily: "var(--font-h)", color: "#6BAA75" }}>{fmt(weeklyDigest.earned)}</div></div>{weeklyDigest.topCat && <div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-h)" }}>Top Category</div><div style={{ fontSize: 13, fontWeight: 600, fontFamily: "var(--font-h)", color: "var(--text)" }}>{cats.find(c => c.id === weeklyDigest.topCat[0])?.name || weeklyDigest.topCat[0]} · {fmt(weeklyDigest.topCat[1])}</div></div>}</div></div>}
