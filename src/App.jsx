...
const sbGet = async (table) => {
  ...
  try {
    ...
    if (r.status === 400 && filter) {
      ...
      const rows2 = await r2.json(); saveVersions(table, rows2); return rows2;
    }
...
export default function Nomad() {
  ...
  const parseBankCsv = (text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!rawDate) continue;
      const parsedDate = (() => { const m = rawDate.match(/^(\d{2})[/-](\d{2})[/-](\d{2,4})$/); if (m) { const y = m[3].length === 2 ? "20" + m[3] : m[3]; return `${y}-${m[2].padStart(2,"0")}-${m[1].padStart(2,"0")}`; } if (/^\d{4}[/-]/.test(rawDate)) { const d = new Date(rawDate); if (!Number.isNaN(d.getTime())) return localDateKey(d); } return null; })();
  ...
  const _doSaveSchedule = async () => {
    ...
    if (reportFreq === "weekly") {
      ...
      next.setUTCDate(dom);
      if (now.getUTCDate() > dom || (now.getUTCDate() === dom && nowISTH >= reportSendHour)) { next.setUTCDate(1); next.setUTCMonth(next.getUTCMonth() + 1); next.setUTCDate(Math.min(reportSendDom, new Date(Date.UTC(next.getUTCFullYear(), next.getUTCMonth() + 1, 0)).getUTCDate())); }
    } else if (reportFreq === "quarterly") {
      const dom = Math.min(reportSendDom, new Date(Date.UTC(next.getUTCFullYear(), next.getUTCMonth() + 1, 0)).getUTCDate());
      next.setUTCDate(dom);
      if (now.getUTCDate() > dom || (now.getUTCDate() === dom && nowISTH >= reportSendHour)) { next.setUTCDate(1); next.setUTCMonth(next.getUTCMonth() + 3); next.setUTCDate(Math.min(reportSendDom, new Date(Date.UTC(next.getUTCFullYear(), next.getUTCMonth() + 1, 0)).getUTCDate())); }
    } else {
