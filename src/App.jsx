...
export default function Nomad() {
  ...
  const confirmCsvImport = () => { if (!csvPreview?.length) return; let imported = 0; const batch = makeBatchTracker(), today = localDateKey(); const defWallet = wallets[0]?.id || "bank"; const defCat = cats[0]?.id || "food"; const defSrc = isrc[0]?.id || "allowance"; csvPreview.forEach(row => { const amount = roundMoney(Number(row.amount) || 0); const isBackdated = row.date < today; const balanceDelta = batch.deltaFor(defWallet, row.date, isBackdated); const ok = row.type === "income" ? addI({ id: uid(), amount, sourceId: defSrc, walletId: defWallet, date: row.date, note: (row.note || "").slice(0, 500) }, { balanceDelta }) : addE({ id: uid(), amount, categoryId: defCat, walletId: defWallet, date: row.date, note: (row.note || "").slice(0, 500) }, { balanceDelta }); if (ok !== false) { imported++; batch.record(defWallet, row.date, row.type === "income" ? amount : -amount); } }); sCsvPreview(null); showT(`Imported ${imported} transactions — recategorize as needed`, "success"); };
