...
import { FilmSlate, ForkKnife, Airplane, GameController, ShoppingCart, MusicNote, Trophy, Confetti, BookOpen, Briefcase, Warning, Wallet, Target, Lightning, Envelope, Fire, Sparkle, Lightbulb, ClipboardText, Timer, HandWaving, BellSlash, Robot, Receipt, FilePdf, Trash, Moon, Sun, Scales, Gear, PushPin, Hash, Microphone, CheckCircle, Circle, ArrowsLeftRight } from "@phosphor-icons/react";
...
function Splits({ splits: sp, settlements: stl, categories: cats = [], onAdd, onSettle: os, onSettleNet: osn = () => {}, onDelete: od, onSkip: ok, expanded: exp, onToggle: ot, wallets: wl = WALLETS, walletBalances: wBalances = {}, onError = () => {} }) {
  ...
  if(exp&&drill){
    ...
    const hasOweSide=pendingSplits.some(s=>s.direction==="owe"&&remainForSplit(s)>0.005); const hasOwedSide=pendingSplits.some(s=>s.direction==="owed"&&remainForSplit(s)>0.005); const mixedSides=hasOweSide&&hasOwedSide; const nonUpiWallets=wl.filter(w=>!isUpiLite(w)); const defaultNetWid=(nonUpiWallets[0]||wl[0])?.id;
    const events=[...pm.splits.map(s=>({...s,_k:"split"})),...pStls.map(s=>({...s,_k:"stl"}))].sort((a,b)=>itemTimestamp(b)-itemTimestamp(a));
    return <div style={{...cc,borderRadius:16,padding:18,marginBottom:14,position:"relative",overflow:"hidden"}}><div style={{position:"absolute",bottom:0,right:0,width:60,height:3,borderRadius:"3px 0 0 0",background:"#D4726A"}}/><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}><button onClick={()=>{sDrill(null);sDelConfirm(null);}} style={{background:"none",border:"none",cursor:"pointer",color:"var(--muted)",padding:2,display:"flex",alignItems:"center"}}><IconChevronLeft size={18}/></button><div style={{width:36,height:36,borderRadius:"50%",background:aColor,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontFamily:"var(--font-h)",fontSize:13,fontWeight:700,color:"#fff"}}>{initials(drill)}</span></div><div style={{flex:1}}><div style={{fontFamily:"var(--font-h)",fontSize:15,fontWeight:700,color:"var(--text)"}}>{drill}</div><div style={{fontSize:11,fontFamily:"var(--font-h)",fontWeight:700,color:Math.abs(net)<0.01?"var(--muted)":net>0?"#6BAA75":"#E07A5F"}}>{Math.abs(net)<0.01?"Fully settled ✓":net>0?`Owes you ${fmt(net)}`:`You owe ${fmt(-net)}`}</div></div>{mixedSides&&settleNetWid===null&&settleAllWid===null&&<button onClick={()=>sSNW(defaultNetWid)} style={{padding:"6px 10px",border:"1.5px solid #7B8CDE",borderRadius:8,background:"#7B8CDE12",color:"#7B8CDE",fontFamily:"var(--font-h)",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",gap:4}}><ArrowsLeftRight size={12} weight="bold"/>Settle net</button>}{pendingSplits.length>1&&!mixedSides&&settleAllWid===null&&settleNetWid===null&&<button onClick={()=>sSAW("bank")} style={{padding:"6px 10px",border:"1.5px solid #6BAA75",borderRadius:8,background:"#6BAA7512",color:"#6BAA75",fontFamily:"var(--font-h)",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>Settle all</button>}</div>
    {settleNetWid!==null&&<div style={{background:"var(--bg)",borderRadius:12,padding:14,marginBottom:12,border:"1.5px solid #7B8CDE"}}><div style={{fontFamily:"var(--font-h)",fontSize:12,fontWeight:700,color:"var(--text)",marginBottom:4}}>{Math.abs(net)<0.005?`Settle up with ${drill}`:net<0?`Pay net ${fmt(-net)} to ${drill}`:`Collect net ${fmt(net)} from ${drill}`}</div><div style={{fontSize:10.5,color:"var(--muted)",fontFamily:"var(--font-b)",marginBottom:10,lineHeight:1.4}}>Clears all {pendingSplits.length} IOUs at once — the owe/owed amounts cancel out and only the net {net<0?"leaves":"enters"} this wallet.</div><div style={{fontSize:9,fontFamily:"var(--font-h)",color:"var(--muted)",fontWeight:600,marginBottom:6,letterSpacing:"0.5px"}}>{net<0?"PAY FROM":"RECEIVE INTO"}</div><div style={{display:"flex",gap:6,marginBottom:10}}>{nonUpiWallets.map(w=><button key={w.id} onClick={()=>sSNW(w.id)} style={{flex:1,padding:"8px 4px",borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",gap:3,border:`2px solid ${settleNetWid===w.id?w.color:"var(--border)"}`,background:settleNetWid===w.id?w.color+"15":"var(--card)",cursor:"pointer"}}><DI2 id={w.id} accent={w.neon||w.color} size={16}/><span style={{fontSize:8,fontFamily:"var(--font-h)",fontWeight:settleNetWid===w.id?700:500,color:settleNetWid===w.id?w.color:"var(--muted)"}}>{w.name}</span></button>)}</div><div style={{display:"flex",gap:8}}><button onClick={()=>sSNW(null)} style={{flex:1,padding:"9px",border:"1.5px solid var(--border)",borderRadius:9,background:"transparent",color:"var(--muted)",fontFamily:"var(--font-h)",fontSize:12,cursor:"pointer"}}>Cancel</button><button onClick={(ev)=>{if(ev.currentTarget.disabled)return;ev.currentTarget.disabled=true;const ok=osn(drill,settleNetWid);if(ok!==false){sSNW(null);}else{ev.currentTarget.disabled=false;}}} style={{flex:2,padding:"9px",border:"none",borderRadius:9,background:"#7B8CDE",color:"#fff",fontFamily:"var(--font-h)",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}><IconCheck size={14}/>{Math.abs(net)<0.005?"Confirm settled":net<0?`Pay ${fmt(-net)}`:`Collect ${fmt(net)}`}</button></div></div>}
...
function LedgerM({ wallet: w, rows, curBal, onReconcile, onClose: cl }) {
  // Per-row reconciliation marker. Tapping the left badge stamps a row as
  // "verified against statement"; persists across sessions in localStorage
  // keyed by tx id (ids are globally unique, so the set is shared).
  const VKEY = "nomad-ledger-verified-v1";
  const [verified, setVerified] = useState(() => { try { return new Set(JSON.parse(localStorage.getItem(VKEY) || "[]")); } catch { return new Set(); } });
  const toggleV = id => { if (!id) return; setVerified(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); try { localStorage.setItem(VKEY, JSON.stringify([...n])); } catch { /* quota */ } return n; }); };
  return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={cl}>
    <div onClick={e => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: "20px 20px 0 0", padding: "20px 20px 24px", width: "100%", maxWidth: 430, maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
      ...
      <p style={{ fontSize: 12.5, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 14, lineHeight: 1.5 }}>Compare top-down against your bank/UPI statement. The first row where NOMAD and your statement disagree is where an entry is missing. Tap the circle to mark a row verified. NOMAD balance: <strong style={{ color: w.color }}>{fmt(curBal)}</strong>.</p>
      <div style={{ flex: 1, overflowY: "auto", margin: "0 -4px" }}>
        {rows.length === 0 ? <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--muted)", fontSize: 13 }}>No transactions on this wallet yet.</div> : rows.map((r, i) => { const isV = verified.has(r.id); return <div key={r.id || i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 4px", borderBottom: i < rows.length - 1 ? "1px solid var(--border)" : "none" }}>
          <button onClick={() => toggleV(r.id)} title={isV ? "Verified — tap to clear" : "Mark verified against statement"} aria-label={isV ? "Verified" : "Not verified"} style={{ flexShrink: 0, background: "none", border: "none", padding: 2, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 0 }}>{isV ? <CheckCircle size={18} weight="fill" color="#6BAA75" /> : <Circle size={18} color="var(--border)" />}</button>
