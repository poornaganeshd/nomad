...
import {
  ...
  recurringDaysOverdue, distributeAmount, groupShareTotals, historySortCompare, itemTimestamp, netAcrossPeople, writeOffTotals,
} from "./financeUtils";
...
export default function Nomad() {
  ...
  const [netOpen, sNetOpen] = useState(true);
  // Write-off tags live in their OWN store, never on the synced split row, so a
  // background Supabase pull can't wipe them and no DB column is needed.
  const [writeOffs, sWriteOffs] = useState(() => { try { return JSON.parse(localStorage.getItem("nomad-writeoffs-v1") || "{}"); } catch { return {}; } });
  useEffect(() => { try { localStorage.setItem("nomad-writeoffs-v1", JSON.stringify(writeOffs)); } catch { /* quota */ } }, [writeOffs]);
  ...
  // Push categories / income sources / write-off tags to user_prefs (debounced)
  // whenever they change. Gated on prefsSync==="on" so it's a no-op until the
  // table is confirmed present. A single JSONB row keyed "nomad".
  useEffect(() => {
    ...
    prefsSaveRef.current = setTimeout(() => {
      sbUpsert("user_prefs", [{ key: "nomad", value: { categories: cats, incomeSources: isrc, writeOffs }, updated_at: new Date().toISOString() }], "user_prefs:nomad");
    }, 1200);
    return () => { if (prefsSaveRef.current) clearTimeout(prefsSaveRef.current); };
  }, [cats, isrc, writeOffs, loaded, prefsSync]);
        ...
        try {
          ...
          if (rp.ok) {
            ...
            if (pv && !hasPendingDedupeKey("user_prefs:nomad")) {
              ...
              if (pv.writeOffs && typeof pv.writeOffs === "object") { const mergedWO = { ...pv.writeOffs, ...writeOffs }; if (JSON.stringify(mergedWO) !== JSON.stringify(writeOffs)) sWriteOffs(mergedWO); }
            }
            // First connect with no prefs row yet: seed it from local setup so the
            // other devices can pull it.
            if (!pv) { const lp0 = (() => { try { return JSON.parse(localStorage.getItem("nomad-v5") || "{}"); } catch { return {}; } })(); sbUpsert("user_prefs", [{ key: "nomad", value: { categories: lp0.categories || cats, incomeSources: lp0.incomeSources || isrc, writeOffs }, updated_at: new Date().toISOString() }], "user_prefs:nomad"); }
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <Splits splits={sp} settlements={stl} categories={cats} expanded={spX} onToggle={() => sSpX(!spX)} onAdd={s => { const sr = { ...s, createdAt: new Date().toISOString() }; sSp(p => [...p, sr]); sbUpsert("splits", [toSB(sr, COLS.splits)]) }} onSettle={settle} onSettleNet={settleNet} onDelete={id => { sSp(p => p.filter(s => s.id !== id)); sbDelete("splits", id); sWriteOffs(m => { if (!(id in m)) return m; const n = { ...m }; delete n[id]; return n; }); }} onSkip={id => { sSp(p => p.map(s => s.id === id ? {...s, settled: true, skipped: true} : s)); sbUpsert("splits", [{id, settled: true, skipped: true}]); showT("IOU skipped ✓"); }} onWriteOff={(id, kind) => { sWriteOffs(m => ({ ...m, [id]: kind })); sSp(p => p.map(s => s.id === id ? { ...s, settled: true } : s)); sbUpsert("splits", [{ id, settled: true }], `splits:${id}`); showT(kind === "written" ? "Written off ✓ — non-cash" : "Forgiven ✓ — non-cash"); }} wallets={wallets} walletBalances={wBal} onError={msg => showT(msg, "error")} />
        {(() => { const wo = writeOffTotals(sp, stl, writeOffs); if (wo.written <= 0.005 && wo.forgiven <= 0.005) return null; const f2 = n => "₹" + (Number(n) || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }); return <div style={{ ...cc, padding: "10px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}><Scales size={16} color="#D4726A" weight="bold" style={{ flexShrink: 0 }} /><div style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--ts)", fontFamily: "var(--font-b)" }}><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, color: "#D4726A", marginRight: 6, letterSpacing: "0.3px" }}>Write-offs</span>{wo.written > 0.005 && <span>written off <strong style={{ color: "#E07A5F" }}>{f2(wo.written)}</strong></span>}{wo.written > 0.005 && wo.forgiven > 0.005 && " · "}{wo.forgiven > 0.005 && <span>forgiven <strong style={{ color: "#6BAA75" }}>{f2(wo.forgiven)}</strong></span>}</div><div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 13.5, fontWeight: 800, color: "var(--text)" }}>{wo.netLoss > 0.005 ? "−" : wo.netLoss < -0.005 ? "+" : ""}{f2(Math.abs(wo.netLoss))}</div><div style={{ fontSize: 8.5, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 600, letterSpacing: "0.3px" }}>NET · NON-CASH</div></div></div>; })()}
        {(() => { const na = netAcrossPeople(sp, stl, evs); if (!na.people.length) return null; const aClr = "#7B8CDE"; return <div style={{ ...cc, padding: "12px 14px", marginBottom: 14, borderLeft: `3px solid ${aClr}` }}><div onClick={() => sNetOpen(o => !o)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", marginBottom: netOpen ? 10 : 0 }}><div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}><ArrowsLeftRight size={14} weight="bold" color={aClr} /><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: aClr, fontWeight: 700, letterSpacing: "0.5px" }}>Net across everything</div></div><div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}><span style={{ fontSize: 11, fontFamily: "var(--font-h)", fontWeight: 700 }}>{na.totalOwed > 0.005 && <span style={{ color: "#6BAA75" }}>+{fmt(na.totalOwed)}</span>}{na.totalOwed > 0.005 && na.totalOwe > 0.005 && <span style={{ color: "var(--muted)" }}> · </span>}{na.totalOwe > 0.005 && <span style={{ color: "#E07A5F" }}>-{fmt(na.totalOwe)}</span>}</span><span style={{ fontSize: 10, color: "var(--muted)", display: "inline-block", transition: "transform 0.2s", transform: netOpen ? "rotate(0deg)" : "rotate(-90deg)" }}>▾</span></div></div>{netOpen && na.people.map(p => <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 11, padding: "9px 2px", borderTop: "1px solid var(--border)" }}><div style={{ width: 38, height: 38, borderRadius: "50%", background: avatarColor(p.name), display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, color: "#fff" }}>{initials(p.name)}</span></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 14.5, fontWeight: 700, color: "var(--text)" }}>{p.name}</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 1 }}>{p.placeCount} place{p.placeCount === 1 ? "" : "s"}{(p.owe > 0.005 || p.owed > 0.005) ? " · " : ""}{p.owe > 0.005 ? `owe ${fmt(p.owe)}` : ""}{p.owe > 0.005 && p.owed > 0.005 ? " / " : ""}{p.owed > 0.005 ? `owed ${fmt(p.owed)}` : ""}</div></div><div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 800, color: Math.abs(p.net) < 0.005 ? "var(--muted)" : p.net > 0 ? "#6BAA75" : "#E07A5F" }}>{p.net > 0 ? "+" : p.net < 0 ? "-" : ""}{fmt(Math.abs(p.net))}</div><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 600, letterSpacing: "0.3px" }}>{Math.abs(p.net) < 0.005 ? "SETTLED" : p.net > 0 ? "NET OWED YOU" : "YOU NET OWE"}</div></div></div>)}</div>; })()}
