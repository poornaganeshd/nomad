...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        {/* Upcoming recurring bills — anything due in the next 31 days that
            isn't already due today. Without this, a monthly bill is invisible
            on Home until its exact due day; now the next occurrence always
        ...
        {(() => {
          ...
          return up.length > 0 && <div style={{ ...cc, padding: "14px 16px", marginBottom: 14 }}>
            ...
            {up.map(({ r, due }) => { const cat = cats.find(c => c.id === r.categoryId) || { name: r.categoryId, color: "#7B8CDE" }; const wal = wallets.find(w => w.id === r.walletId) || { name: r.walletId }; const days = Math.round((new Date(due + "T12:00:00") - new Date(todS + "T12:00:00")) / 86400000); const when = days === 1 ? "tomorrow" : `in ${days} days`; const dd = new Date(due + "T12:00:00").toLocaleDateString(undefined, { day: "numeric", month: "short" }); return <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderTop: "1px solid var(--border)" }}><DI2 id={r.categoryId} accent={cat.neon || cat.color} size={18} /><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.name}</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 1 }}>{wal.name} · {dd} · {when}</div></div><span style={{ fontSize: 14, fontFamily: "var(--font-h)", fontWeight: 700, color: "var(--text)" }}>{fmt(r.amount)}</span></div>; })}
          </div>;
        })()}
