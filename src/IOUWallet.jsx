...
export default function IOUWallet({ splits = [], settlements = [], categories = [], wallets = [], events = [], fmt = n => "₹" + n, uid = () => Math.random().toString(36).slice(2), isUpiLite = () => false, SettleModal = null, onAdd = () => {}, onSettle = () => {}, onSettleNet = () => {}, onSettleEventNet = () => {}, onSkip = () => {}, onUnskip = () => {}, onDelete = () => {}, onRenamePerson = () => {}, onError = () => {}, focusPerson = null, onFocusHandled = () => {} }) {
  ...
  const settleAllWith = (name, groups, wid, amt, opts) => onSettleNet(name, wid, amt, { general: groups.some(g => !g.eventId), eventIds: groups.filter(g => g.eventId).map(g => g.eventId) }, opts);
...
function NetSheet({ desc, wallets, fmt, isUpiLite, onConfirm, onClose }) {
  ...
  // "Write off the rest": a partial net settle used to strand the unpaid tail —
  // paying ₹240 of ₹300 left ₹60 pending with no way to close it except
  // skipping each leftover IOU one by one. Ticking this settles what moved and
  // writes the remainder into the write-off ledger (reversible per IOU).
  const [forgive, sForgive] = useState(false);
  ...
  // AMOUNT ZERO = write the whole thing off. Zero is not the same as an empty
  // field (which still means "settle the full net"), and it used to be treated
  // as one: typing 0 recorded the entire net as if it had been paid. It is also
  ...
  const blockUpiLite = !zeroed && settleWritesIncoming({ net: n, hasOwedItems: desc?.hasIncoming, partial });
      ...
      <div style={{ padding: 20 }}>
        ...
        {(partial || zeroed) && <button onClick={() => sForgive(f => !f)} aria-pressed={forgive} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, textAlign: "left", border: "none", borderRadius: 14, padding: "11px 13px", margin: "-4px 0 14px", cursor: "pointer", background: SURF, boxShadow: forgive ? NEU_INSET : NEU_SM, transition: "box-shadow .15s" }}>
          ...
          <span style={{ minWidth: 0 }}>
            <span style={{ display: "block", fontFamily: "var(--font-h)", fontWeight: 800, fontSize: 12, color: "var(--text)" }}>{zeroed ? `Write off the whole ${fmt(absNet)}` : `Write off the remaining ${fmt(roundMoney(absNet - roundMoney(entered)))}`}</span>
