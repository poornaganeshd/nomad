...
- **Tests:** 744 pass / 0 fail, 36 files (`npm test`). E2E: 50 pass, 15 files (`npm run test:e2e`).
...
| `billReminders.js` | due/upcoming recurring-bill + outstanding-IOU reminders; exports `isNotHandled` (paid/skipped-this-cycle check). IOUs are aggregated **per person** on the **remaining** balance (settlements subtracted, `excess` ignored), skipping settled/skipped/soft-deleted rows — pass `settlements` as the 6th arg |
