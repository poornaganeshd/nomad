...
- **Tests:** 814 pass / 0 fail, 38 files (`npm test`). E2E: 72 pass, 18 files (`npm run test:e2e`).
...
- **Quick-add chips WRAP and cap their width** (`maxWidth: 172` + ellipsized note). A long note (`"Nippon multicap (recurring…"`) used to make an unbounded chip inside a hidden-scrollbar `overflow-x` row, so the last chip was sliced off mid-word at the viewport edge with no affordance that it scrolled. Render count matches what `quickPatterns` actually produces (5, not the old dead `slice(0, 8)`). Guarded by e2e `17-people-search.spec.js`.
...
- **`alpha()` parses HEX — never hand it a `var(--…)`.** `alpha("var(--acc)", .13)` doesn't throw, it silently returns `rgba(0,0,0,.13)`: the parse yields `NaN`, and the `|| 0` fallback turns every channel black. The whole "Split with friends" panel was tinted translucent BLACK for exactly this reason while looking like it was using the accent. Keep a hex twin next to the token when a section needs both (`SPL = "var(--acc)"` for solid colors, `SPL_A = "#7B8CDE"` for `alpha()`), the way `acc/grn/ind/gld` already do.
