...
Optional bill/IOU reminders as real phone push via **ntfy** (`src/ntfy.js`, tests `src/__tests__/ntfy.test.js`). No Web Push/VAPID, no new env vars — the browser POSTs straight to the ntfy server (ntfy.sh is CORS-open and whitelisted in the `vercel.json` CSP `connect-src`; a custom server must be added there too). Two legs, same de-dupe as the in-app toast: **client-side** (`App.jsx` bill-reminders effect mirrors `checkBillReminders()` output to `publishNtfy` — fires while a tab is open) and **server-side** (`api/_notify.ts` + the `send-reports` cron push a due-bill digest once per IST day, deduped via `notification_prefs.last_run_date`, so reminders arrive with the app closed). Config lives in localStorage `nomad-ntfy-v1` AND, when Supabase is connected, the per-user `notification_prefs` row (Settings → "Push Notifications (ntfy)" card; Save also registers the user in `user_registry`). `api/_notify.ts` is a hand-port of `financeUtils.js` recurring-due math (api/ is CommonJS, can't import ESM `src/`) — change both together; guarded by `api/__tests__/notify.test.ts`.
